...
class Word2Vec(object):
  ...
  def cosine_distance(self, w0, w1):
    emb_w0 = self.get_embed(w0)
    emb_w1 = self.get_embed(w1)
    dot = tf.reduce_sum(tf.mul(emb_w0, emb_w1))
    norm_w0 = tf.square(tf.reduce_sum(tf.square(emb_w0)))
    norm_w1 = tf.square(tf.reduce_sum(tf.square(emb_w1)))
    print(self._session.run(dot / (norm_w0 * norm_w1))) 
